...
| 3D-Modelle Ist/Soll | Liegen als JSON im Repo (`public/models/`), werden mit dem App-Build ausgeliefert und vom Service Worker vorgecacht. Austausch = Datei ersetzen + `git push`. Vollständig dokumentiert für den "Modell-Agenten" (Abschnitt 9). |
...
- **Neu: Modell-Umschalter** oben: Segment-Control **"Ist" | "Soll"** (Default aus Einstellungen, URL-Param `variant`). Wechsel lädt die andere JSON (aus Precache/IndexedDB), baut die Szene neu, behält Kamera + Layer-Zustand. Versionsinfo (aus `manifest.json`: `version`, `updatedAt`, `note`) klein im Header, z. B. "Ist v0.22 · 13.09.2026".
...
- Modell laden: `fetch(`${base}models/${variant}.json`)` → JSON parsen (≈180 KB, 132 Bauteile, 3088 Dreiecke – unkritisch). Ladefehler offline → Meldung "Modell noch nicht heruntergeladen – einmal online öffnen".
...
- Modelle: Tabelle Ist/Soll mit Version, Datum, Notiz (aus `manifest.json`), Standardvariante.
