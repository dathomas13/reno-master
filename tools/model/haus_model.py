...
Quellen:
  ...
  KG  = eigenes Aufmaß Thomas 09/2026, DXF "Grundriss_KG_Bestand_Fertigmasse".
        **Fertigmaße inklusive Putz**, außer Nordzeile und Heizung: die sind unverputzt,
        dort steht das Rohmaß. Wandkanten und Öffnungen aus der Zeichnung.
  OG  = eigenes Aufmaß Thomas 09/2026, DXF "Grundriss_OG_Bestand_Fertigmasse".
        Aufgemessen sind Kind 1-3, HWR und Diele; Bad, Garderobe und Wäscheboden sind aus
        Blatt 4 übernommen und im Plan mit * als noch zu prüfen markiert -> Tag B.
        Die Abseitenwände gibt es nur bei HWR (vollständig) und Kind 1 (Raum dahinter zur
        Seite offen); bei Kind 2 und Kind 3 wurden sie nie gebaut.
...
# -------- KG (Aufmaß 09/2026, DXF "Grundriss_KG_Bestand_Fertigmasse") -----------
# Fertigmaße inkl. Putz. Nordzeile und Heizung sind unverputzt, dort steht das Rohmaß
# (Außenwand 365 statt 400) - so ist es im Plan vermerkt und so ist es gezeichnet.
W("KG", "Außenwand Süd",            400,     0, 12595,   400, "A")
W("KG", "Außenwand West",             0,     0,   400,  7610, "A")
W("KG", "Außenwand West (roh)",       0,  7610,   365, 11815, "A")
W("KG", "Außenwand Nord",           365, 11450, 12630, 11815, "A")
W("KG", "Außenwand Ost",          12595,     0, 12995,  7140, "A")
W("KG", "Außenwand Ost (roh)",    12630,  7140, 12995, 11815, "A")
# tragende Innenwände - die Achsen liegen unter denen des EG, aber gemessen 245/250/270
W("KG", "Tragwand y=4860 (Süd)",    400,  4860,  8505,  5130, "A")
W("KG", "Tragwand y=7365 (Nord)",   365,  7365,  8235,  7610, "A")
W("KG", "Pfeiler Tragwand Nord",   3640,  7365,  3800,  7610, "A")   # Pfeiler neben der Tür
W("KG", "Tragwand x=4745 (Diele West)", 4745, 5130, 5015, 7610, "A")
W("KG", "Tragwand x=8235 (Diele Ost)",  8235,  400, 8505, 7020, "A")
W("KG", "Tragwand x=8235 (Kellerflur|Heizung)", 8235, 7020, 8470,  8930, "A")
W("KG", "Tragwand x=4760 (Obst|Keller 2)",      4760, 8930, 5000, 11450, "A")
W("KG", "Tragwand x=8220 (Keller 2|Öllager)",   8220, 8930, 8470, 11450, "A")
W("KG", "Schlafzimmer Nord 27",    8505,  4200, 12595,  4470, "A")
# nichttragend (11,5 + Putz = 12,5 / 14 / 15 lt. Legende)
W("KG", "Essküche|Wohnzimmer 12,5", 2710,   400,  2835,  4860, "A")
W("KG", "Keller 1|Kellerflur 14",   3500,  7610,  3640, 11450, "A")
W("KG", "Nordzeile Süd 14",         3640,  8790, 12630,  8930, "A")
W("KG", "Flur|Bad 15",             10105,  4470, 10255,  7020, "A")
W("KG", "Flur/Bad|Heizung 12 West", 8505,  7020,  9000,  7140, "A")
W("KG", "Flur/Bad|Heizung 12 Ost", 10625,  7020, 12630,  7140, "A")
W("KG", "Vorsatz vor Kamin 3,5",    9000,  7020, 10625,  7055, "A")
# Kamin 162,5 x 51, durchgehend in allen drei Geschossen (Plannotiz "wie EG")
W("KG", "Kamin", 9000, 7055, 10625, 7565, "A")

# -------- OG (Aufmaß 09/2026, DXF "Grundriss_OG_Bestand_Fertigmasse") -----------
# Aufgemessen: Kind 1-3, HWR, Diele. Bad, Garderobe und Wäscheboden sind im Plan mit *
# markiert (aus Blatt 4 übernommen, noch zu prüfen) -> Tag B.
# Die Abseitenwände: bei Kind 2 und Kind 3 nie gebaut, bei HWR vollständig vorhanden,
# bei Kind 1 vorhanden, der Raum dahinter aber zur Seite offen bis zum Wäscheboden.
W("OG", "Außenwand Süd",            400,     0,  4840,   400, "A")
W("OG", "Außenwand Süd (Erker)",   4840,     0,  9370,   365, "A")
W("OG", "Außenwand Süd (Ost)",     9370,     0, 12595,   400, "A")
W("OG", "Außenwand Nord",           400, 11415, 12595, 11815, "A")
W("OG", "Außenwand West",             0,     0,   400, 11815, "A")
W("OG", "Außenwand Ost",          12595,     0, 12995, 11815, "A")
# Längswände: die beiden durchgehenden Trennwände (17,5 bzw. 13,5 mit Putz)
W("OG", "Westspalte | Mitte 17,5", 4840,   365,  5015, 10570, "A")
W("OG", "Mitte | Ostspalte 13,5",  9235,   365,  9370,  6955, "A")
# Querwände
W("OG", "Kind 1 | Kind 2 12",       400,  5800,  4840,  5920, "A")
W("OG", "HWR | Wäscheboden 12",    9370,  5800, 12595,  5920, "A")
W("OG", "Kind 3 Nord 16",          5015,  4545,  9235,  4705, "A")
W("OG", "Abseitenwand HWR 12",     9370,  1570, 12595,  1690, "A")
W("OG", "Abseitenwand Kind 1 12",   400, 10450,  6025, 10570, "A")
W("OG", "Treppe | Bad 12",         6025,  6835,  6145, 10570, "A")
W("OG", "Diele | Bad 12",          6945,  6835,  9370,  6955, "B")
W("OG", "G | Bad 12",              6945,  6955,  7065,  7625, "B")
W("OG", "G Nord 12",               6145,  7505,  7065,  7625, "B")
W("OG", "Bad Nord 12",             6145,  8575,  9000,  8695, "B")
W("OG", "Bad | Wäscheboden 12",    8880,  6955,  9000,  8575, "B")
W("OG", "Kamin", 9000, 7055, 10625, 7565, "A")
...
# -------- KG: Lage und Breite aus dem Aufmaß, Brüstungs- und Sturzhöhen offen (C)
O("KG", "Außenwand Süd", "window",   965, 5510, 900, 1260, "A")   # Fensterelement durchgehend,
#   die Trennwand Essküche|Wohnzimmer stößt an das Element an (Plannotiz)
O("KG", "Außenwand Süd", "window",  9145, 2010, 900, 1260, "A")   # Schlafzimmer
O("KG", "Außenwand Nord", "window", 1375,  800, 1800, 600, "A")   # Keller 1
O("KG", "Außenwand Nord", "window", 5845,  800, 1800, 600, "A")   # Keller 2
O("KG", "Außenwand Ost", "window",  5570, 1250, 900, 1260, "A")   # Bad
O("KG", "Außenwand Ost (roh)", "window", 1935, 680, 1800, 600, "A")   # Öllager
O("KG", "Außenwand West", "door",   5620, 1260, 1375, 1235, "B")  # Haustür, unterer Teil
#   (im KG-Plan nicht gezeichnet - liegt über der Schnittebene; Maß aus dem EG-Aufmaß)
O("KG", "Tragwand y=4860 (Süd)", "door", 6100, 1010, 0, 2010, "A")
O("KG", "Tragwand y=7365 (Nord)", "door", 3435, 800, 0, 2010, "A")
O("KG", "Tragwand x=4745 (Diele West)", "door",  170, 1010, 0, 2010, "A")
O("KG", "Tragwand x=8235 (Diele Ost)", "door",  5400,  885, 0, 2010, "A")
O("KG", "Tragwand x=8235 (Kellerflur|Heizung)", "door", 780, 800, 0, 2010, "A")
O("KG", "Schlafzimmer Nord 27", "door",  625, 885, 0, 2010, "A")
O("KG", "Essküche|Wohnzimmer 12,5", "door", 3200, 885, 0, 2010, "A")
O("KG", "Keller 1|Kellerflur 14", "door",    190, 800, 0, 2010, "A")
O("KG", "Nordzeile Süd 14", "door",  165, 760, 0, 2010, "A")   # Kellerflur|Obst
O("KG", "Nordzeile Süd 14", "door", 1660, 800, 0, 2010, "A")   # Kellerflur|Keller 2
O("KG", "Nordzeile Süd 14", "door", 5205, 885, 0, 2010, "A")   # Heizung|Öllager
O("KG", "Flur|Bad 15", "door", 1630, 760, 0, 2010, "A")

# -------- OG: Lage und Breite aus dem Aufmaß, Höhen aus Blatt 4 (C)
O("OG", "Westspalte | Mitte 17,5", "door", 4490, 885, 0, 2010, "A")   # Diele|Kind 2
O("OG", "Westspalte | Mitte 17,5", "door", 5650, 885, 0, 2010, "A")   # Diele|Kind 1
O("OG", "Mitte | Ostspalte 13,5", "door",  4490, 885, 0, 2010, "A")   # Diele|HWR
O("OG", "Mitte | Ostspalte 13,5", "door",  5575, 885, 0, 2010, "A")   # Diele|Wäscheboden
O("OG", "Kind 3 Nord 16", "door", 1125, 885, 0, 2010, "A")
O("OG", "Diele | Bad 12", "door",  240, 760, 0, 2010, "B")
O("OG", "Außenwand West", "door", 3730, 2010, 0, 2135, "A")   # Balkontür Kind 2
O("OG", "Außenwand West", "door", 5980, 2010, 0, 2135, "A")   # Balkontür Kind 1
O("OG", "Außenwand Ost", "window", 4605, 1135, 900, 1300, "A")   # HWR (Höhe C)
O("OG", "Außenwand Ost", "window", 5980, 1135, 900, 1300, "A")   # Wäscheboden (Höhe C)
# Garage
O("GAR", "Garage Süd", "door", 240+365, 2300, 0, 2100, "A")        # Tor 1
O("GAR", "Garage Süd", "door", 240+365+2300+365, 2300, 0, 2100, "A")  # Tor 2

# ---------------------------------------------------------------- Gaube (Blatt 4/5)
# Erker/Gaube Süd: Breite und Fensterteilung aus dem OG-Aufmaß (Außenwand 36,5 | 17 | 14
# = 675 Aufbau, Wange West 175, Ost 135, vier Fenster 1010 mit 60er Pfosten).
# Tiefe und Wandhöhe stehen im Grundriss nicht - weiter aus Blatt 4/5 (Tag B).
GAUBE = dict(x0=4840, x1=9370, depth=2250, wall_h=2200, windows=[(1010, 60)] * 4,
             cheek=(175, 135), tag="B")
# Balkon West (OG)
...
STAIRS = [
    ...
    # Holztreppe EG→OG in der Diele, gerader Lauf nach Süden. Der OG-Plan nennt
    # 14 Stg 19,6/27 und als Austritt die Nordkante der Tür Kind 1 (y 6900). Antritt
    # y=10470 (EG-Aufmaß) + 13 Auftritte à 270 ergibt Austritt 6960 - 60 daneben, der
    # Plan markiert die Angabe selbst als noch zu prüfen. Breite 1010 (OG-Aufmaß).
    dict(name="Holztreppe EG→OG", x0=X_LOAD1[1], y0=10470, width=1010, steps=14,
         rise=STOREY / 14, run=270, z0=Z_EG, direction="-y", tag="B"),
]
LANDINGS = [
...
SLAB_OPENINGS = {
    ...
    # Decke über EG: Treppenauge über der Holztreppe, x 5015–6025 aus dem OG-Aufmaß
    # (Treppenwange bis Wand "Treppe | Bad"), Länge 3360 lt. Plannotiz "Deckenöffnung 3,36".
    "OG": (X_LOAD1[1], 6690, 6025, 10050),
}
